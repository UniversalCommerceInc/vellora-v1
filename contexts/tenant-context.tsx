...
const TenantContextProvider = createContext<TenantContext | undefined>(
  undefined
);
...
export function TenantProvider({ children }: TenantProviderProps) {
  ...
  const refreshContext = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "✅ TenantContext: User session found for user:",
        session.user.id
      );
      ...
      if (testError) {
        console.error(
          "❌ TenantContext: Database connectivity test failed:",
          testError
        );
        setTenant(null);
        setOrganization(null);
      ...
      console.log(
        "✅ TenantContext: Database connectivity test passed:",
        testData
      );

      // Get user's tenant access from the database
      console.log(
        "🔄 TenantContext: Fetching user tenant access for user:",
        session.user.id
      );
      const { data: userTenantAccess, error: accessError } = await supabase
        .from("user_tenant_access")
      ...
      if (accessError) {
        console.error(
          "❌ TenantContext: Error fetching user tenant access:",
          accessError
        );
        console.error("❌ TenantContext: Error details:", {
          message: accessError.message,
      ...
      console.log(
        "📊 TenantContext: User tenant access query result:",
        userTenantAccess
      );
      console.log(
        "📊 TenantContext: Number of tenant access records:",
        userTenantAccess?.length || 0
      );
      ...
      if (!userTenantAccess || userTenantAccess.length === 0) {
        ...
        // Check if the user exists in the users table using maybeSingle
        console.log(
          "🔄 TenantContext: Checking if user exists in users table..."
        );
        const { data: existingUser, error: userCheckError } = await supabase
          .from("users")
          .select("*")
        ...
        if (userCheckError) {
          console.error(
            "❌ TenantContext: Error checking user in users table:",
            userCheckError
          );
          setTenant(null);
          setOrganization(null);
        ...
        if (!existingUser) {
          console.log(
            "🔄 TenantContext: User not found in users table, creating user record..."
          );
          ...
          if (createUserError) {
            console.error(
              "❌ TenantContext: Error creating user record:",
              createUserError
            );
            setTenant(null);
            setOrganization(null);
          ...
          console.log(
            "✅ TenantContext: User record created/updated:",
            newUser
          );
          userRecord = newUser;
        } else {
          console.log(
            "✅ TenantContext: User found in users table:",
            existingUser
          );
        }
        ...
        // Check if user already has tenant access (maybe it was created after our first check)
        console.log(
          "🔄 TenantContext: Double-checking for existing tenant access..."
        );
        const { data: recheckAccess, error: recheckError } = await supabase
          .from("user_tenant_access")
          .select(
        ...
        if (!recheckError && recheckAccess && recheckAccess.length > 0) {
          console.log(
            "✅ TenantContext: Found existing tenant access on recheck:",
            recheckAccess[0]
          );
          const primaryAccess = recheckAccess[0];
          setTenant(primaryAccess.tenant);
        ...
        // Now create a demo tenant and organization for this user
        console.log(
          "🔄 TenantContext: Creating demo tenant and organization..."
        );
        ...
        if (tenantError) {
          console.error(
            "❌ TenantContext: Error creating tenant:",
            tenantError
          );
          setTenant(null);
          setOrganization(null);
        ...
        if (orgError) {
          console.error(
            "❌ TenantContext: Error creating organization:",
            orgError
          );
          setTenant(null);
          setOrganization(null);
        ...
        if (accessError) {
          console.error(
            "❌ TenantContext: Error creating user tenant access:",
            accessError
          );
          setTenant(null);
          setOrganization(null);
      ...
      if (primaryAccess.organization) {
        setOrganization(primaryAccess.organization);
        console.log(
          "✅ TenantContext: Organization set:",
          primaryAccess.organization.name
        );
      } else {
        console.log("⚠️ TenantContext: No organization data in access record");
      }
      ...
      setUserAccess(primaryAccess);
      console.log(
        "✅ TenantContext: User access set with role:",
        primaryAccess.role
      );
    } catch (error) {
      console.error(
        "❌ TenantContext: Unexpected error refreshing tenant context:",
        error
      );
      setTenant(null);
      setOrganization(null);
  ...
  const switchTenant = useCallback(async (tenantId: string) => {
    if (!supabaseConfigured) {
      console.log(
        "⚠️ TenantContext: Cannot switch tenant - Supabase not configured"
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "✅ TenantContext: Successfully switched to tenant:",
        targetAccess.tenant?.name
      );
    } catch (error) {
      console.error("❌ TenantContext: Error switching tenant:", error);
    }
  ...
  useEffect(() => {
    console.log(
      "🚀 TenantContext: Component mounted, starting initial refresh..."
    );
    refreshContext();
      ...
      getSupabaseClient().then((supabase) => {
        if (supabase) {
          ...
          const {
            ...
            console.log(
              "🔔 TenantContext: Auth state changed:",
              event,
              session?.user?.id
            );
            if (event === "SIGNED_IN" || event === "SIGNED_OUT") {
...
/**
 * Hook to get user permissions based on their role and tenant access
 */
export function useUserPermissions() {
  const { userAccess, tenant } = useTenant();

  // Return permissions based on user's role
  const permissions = {
    // Admin permissions
    canManageUsers: userAccess?.role === "admin",
    canManageTenant: userAccess?.role === "admin",
    canDeleteData: userAccess?.role === "admin",
    canManageIntegrations: userAccess?.role === "admin",
    canManageBilling: userAccess?.role === "admin",

    // Manager permissions
    canViewUsers: ["admin", "manager"].includes(userAccess?.role || ""),
    canManageDeals: ["admin", "manager", "sales"].includes(
      userAccess?.role || ""
    ),
    canExportData: ["admin", "manager"].includes(userAccess?.role || ""),

    // General user permissions
    canViewDashboard: Boolean(userAccess?.status === "active"),
    canViewDeals: Boolean(userAccess?.status === "active"),
    canCreateDeals: ["admin", "manager", "sales"].includes(
      userAccess?.role || ""
    ),
    canEditOwnDeals: Boolean(userAccess?.status === "active"),
    canViewContacts: Boolean(userAccess?.status === "active"),
    canCreateContacts: Boolean(userAccess?.status === "active"),

    // Settings access
    canViewSettings: Boolean(userAccess?.status === "active"),
    canEditProfile: Boolean(userAccess?.status === "active"),

    // Organization permissions
    canViewOrganization: Boolean(userAccess?.organization_id),
    canEditOrganization: userAccess?.role === "admin",
  };

  return {
    ...permissions,
    role: userAccess?.role || "guest",
    tenantId: tenant?.id || null,
    organizationId: userAccess?.organization_id || null,
    isActive: userAccess?.status === "active",
    hasAccess: Boolean(userAccess?.status === "active"),
  };
}
